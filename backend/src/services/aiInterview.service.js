...
import {
  ...
  isQuestionRepeated,
} from "./repetitionGuard.service.js";
...
function firstSyllabus(syllabusDocuments = []) {
  return (
    syllabusDocuments[0] || {
      subject: "General",
      topics: ["Interview readiness"],
      sampleConcepts: ["clarity", "correctness"],
      difficulty: "medium",
    }
  );
}
  ...
  for (const item of syllabusDocuments) {
    ...
    if (topic)
      return {
        ...
        topic,
        concepts: item.sampleConcepts || [],
        difficulty: item.difficulty || session.difficulty,
      };
  }
  const fallback = firstSyllabus(syllabusDocuments);
  return {
    subject: fallback.subject,
    topic: fallback.topics?.[0] || "Interview readiness",
    concepts: fallback.sampleConcepts || [],
    difficulty: fallback.difficulty || session.difficulty,
  };
}
...
function fallbackFirstQuestion(session, syllabusDocuments) {
  ...
  const subjectList = (session.selectedSubjects || [])
    .filter(Boolean)
    .join(", ");
  const selected = pickTopic(session, syllabusDocuments);
  const subjectPrefix = subjectList
    ? `Based on your selected subjects ${subjectList}`
    : "Based on the selected subject focus";

  if (
    session.interviewType === "resume" &&
    (resumeProject?.name || resumeSkill)
  ) {
    const focus = resumeProject?.name
      ? `your project ${resumeProject.name}`
      : `your experience with ${resumeSkill}`;
    return {
      questionText: `Welcome to your mock interview. Let's get started. ${subjectPrefix} and your resume details suggest we begin with ${focus}. Can you walk me through what you built, the decisions you personally owned, and one technical challenge you had to solve?`,
      questionType: "main",
      topic: resumeProject?.name || resumeSkill,
      subject: "Resume deep dive",
      expectedConcepts:
        resumeProject?.techStack || [resumeSkill].filter(Boolean),
      difficulty: session.difficulty || "medium",
      reasoning:
        "Fallback resume-aware first question generated from parsed resume data.",
    };
  }
  return {
    questionText: `Welcome to your mock interview. Let's get started. ${subjectPrefix}. ${selected.topic} is a strong place to begin — can you explain the core idea and walk me through a practical example?`,
    questionType: "main",
...
function extractAnswerKeyword(answerTranscript, fallback) {
  if (!answerTranscript) return fallback;
  const words = answerTranscript
  ...
  const sorted = Object.entries(frequency).sort(
...
function fallbackAnswer(
  ...
  const questionKeyword = extractAnswerKeyword(
    answerTranscript,
    currentQuestion?.topic || "the topic",
  );
  const canCross =
    Number(session.crossQuestionCount || 0) <
    Number(session.maxCrossQuestions || 2);
  const uncertain =
    /\b(not sure|maybe|i think|confused|don't know|do not know)\b/i.test(
      answerTranscript,
    );
  const reachedLimit =
    Number(session.currentQuestionIndex || 0) + 1 >=
    Number(session.totalQuestions || 5);
  const selected = pickTopic(session, syllabusDocuments);
  const evaluation = {
    score: Math.max(2, Math.min(8, Math.round(wordCount / 8))),
    strengths:
      wordCount > 20
        ? ["Provided some explanatory detail"]
        : ["Attempted the answer"],
    gaps:
      wordCount < 25
        ? ["Needs more depth and concrete examples"]
        : ["Could connect concepts more explicitly"],
    brief:
      wordCount < 25
        ? "The answer was brief and needs more technical depth."
  ...
  if (uncertain && canCross) {
    return {
      ...
      questionText: `You mentioned ${questionKeyword} but sounded uncertain. What makes ${questionKeyword} unclear, and how would you verify that assumption in a real interview context?`,
  ...
  if (wordCount < 25 && canCross) {
    return {
      ...
      questionText: `You mentioned ${questionKeyword}. Can you explain how ${questionKeyword} applies here and describe one practical tradeoff or example?`,
...
function fallbackFinalEvaluation(session) {
  ...
  const avg = notes.length
    ? notes.reduce((sum, note) => sum + Number(note.score || 0), 0) /
      notes.length
    : 0;
  return {
    ...
    areasNeedingImprovement: notes
      .flatMap((note) => note.gaps || [])
      .slice(0, 3),
    communicationQuality:
      "Communication was understandable; stronger structure and examples would improve interview impact.",
    recommendedPracticePlan: [
      "Practice concise concept explanations",
      "Add examples and edge cases",
      "Review gaps topic by topic",
    ],
    summary:
      "The session was completed and evaluated from the saved question and answer history.",
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a strict JSON-only voice interview engine. The JSON field questionText must sound natural when spoken aloud.",
        },
...
export const aiInterviewService = {
  ...
  async processAnswer(
    ...
    const prompt = promptBuilder.processAnswer(
      session,
      syllabusDocuments,
      currentQuestion,
      answerTranscript,
  ...
  async processAnswerWithRepetitionGuard(
    session,
    syllabusDocuments,
    currentQuestion,
    answerTranscript,
  ) {
    let result = await this.processAnswer(
      session,
      syllabusDocuments,
      currentQuestion,
      answerTranscript,
    ...
    for (
      ...
      attempt < 2 &&
      result.questionText &&
      isQuestionRepeated(result.questionText, session.askedQuestions || []);
      attempt += 1
    ) {
      const similar = (session.askedQuestions || []).find((question) =>
        isQuestionRepeated(result.questionText, [question]),
      );
    ...
    if (
      ...
      const selected = pickTopic(
        {
          ...session,
          askedTopics: [...(session.askedTopics || []), result.topic].filter(
            Boolean,
          ),
        },
        syllabusDocuments,
