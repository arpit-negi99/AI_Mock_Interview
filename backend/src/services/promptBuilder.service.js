...
const typeGoals = {
  core_cse:
    "test fundamentals, correctness, and real-world reasoning across core computer science subjects",
  dsa: "test algorithmic problem solving, complexity analysis, edge cases, and optimization",
  behavioral:
    "test communication, ownership, collaboration, and structured reflection",
  resume:
    "test authenticity, depth, impact, and defensibility of resume claims",
  project:
    "test architecture, tradeoffs, implementation depth, debugging, and scalability reasoning",
};
...
function adaptiveProfile(session) {
  ...
  if (!notes.length)
    return "No scored answers yet. Start approachable and calibrate from the first response.";
  const average =
    notes.reduce((sum, note) => sum + Number(note.score || 0), 0) /
    notes.length;
  if (average >= 8)
    return `Running average ${average.toFixed(1)}/10. Increase difficulty, challenge edge cases, scalability, and tradeoffs.`;
  if (average <= 4.5)
    return `Running average ${average.toFixed(1)}/10. Slow down, test fundamentals, and offer one gentle hint before moving on.`;
...
function selectionContextBlock(session) {
  const subjects = session.selectedSubjects || [];
  const topics = session.selectedTopics || [];
  return [
    `Selected subjects / technologies: ${subjects.length ? subjects.join(", ") : "None provided"}`,
    `Selected topics: ${topics.length ? topics.join(", ") : "None provided"}`,
  ].join("\n");
}
...
function interviewerPersona(session) {
  return [
    ...
    "Ask only one question at a time. Do not ask multiple questions or multi-part questions in a single response.",
    ...
    "Do not provide direct feedback, corrections, or solution explanations to the candidate. Save all evaluation for the final report.",
...
export const promptBuilder = {
  firstQuestion(session, syllabusDocuments) {
    ...
    return [
      ...
      "Selected subject and topic preferences:",
      selectionContextBlock(session),
      ...
      'Begin with exactly "Welcome to your mock interview. Let\'s get started." followed by one clear spoken question that reflects the selected subjects, resume context, and syllabus topic.',
  ...
  processAnswer(
    ...
    const schema = {
      decision:
        "ASK_FOLLOWUP | ASK_CLARIFICATION | NEXT_QUESTION | END_INTERVIEW",
    ...
    const remaining = serializeSyllabus(syllabusDocuments).map((item) => ({
      ...
      topics: item.topics.filter(
        (topic) => !(session.askedTopics || []).includes(topic),
      ),
    }));
    ...
    return [
      ...
      "If the answer contains a specific technical keyword, tool name, project name, or concept, use that keyword when asking a follow-up question.",
      ...
      "Whenever the decision is ASK_FOLLOWUP, the follow-up must explicitly reference a keyword or concept from the candidate's previous answer.",
      jsonOnlyInstruction(schema),
    ]
      .filter(Boolean)
      .join("\n\n");
